...
export const ReplyMessageBubble = (props: ReplyMessageBubbleProps) => {
  ...
  // Use theme-aware colors - accentLight for reply bubble background
  const getBubbleBackgroundColor = () => {
    // Both variants use a light accent background that works in dark mode
  ...
  return (
    <Pressable
      ...
      <Animated.View style={tailwind.style('flex flex-row')}>
        <Animated.View style={[tailwind.style('w-[3px] h-auto rounded-[4px]'), { backgroundColor: colors.accent }]} />
        <Animated.View style={tailwind.style('pl-2.5')}>
          ...
          {hasAttachments ? (
            <Animated.View style={tailwind.style('py-[3px] flex flex-row items-center')}>
              ...
              <Animated.Text
                style={[
                  tailwind.style('text-[14px] font-inter-normal-20 leading-[19.6px] tracking-[0.16px] capitalize pl-1.5'),
          ...
          {replyMessageItem?.content ? (
            isMarkdown(replyMessageItem?.content) ? (
              ...
              <Animated.Text
                numberOfLines={1}
                style={[
                  tailwind.style('text-[14px] font-inter-normal-20 leading-[19.6px] tracking-[0.16px] capitalize'),
                  { color: colors.textPrimary },
                ]}>
                {replyMessageItem?.content}
              </Animated.Text>
